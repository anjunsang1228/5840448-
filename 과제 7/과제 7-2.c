...
int load_students(const char* filename, Student* arr, int n) {
    ...
    for (i = 0; i < n; i++) {
        if (fgets((arr + i)->name, NAME_LEN, fp) == NULL) {
            fclose(fp);
            return 0;
        }
...
Student* get_min_student(Student* arr, int n) {
    int i;
    Student* min_student = arr;

    for (i = 1; i < n; i++) {
        if ((arr + i)->score < min_student->score) {
            min_student = arr + i;
        }
    }
...
Student* get_max_student(Student* arr, int n) {
    int i;
    Student* max_student = arr;

    for (i = 1; i < n; i++) {
        if ((arr + i)->score > max_student->score) {
            max_student = arr + i;
        }
    }
...
int main(void) {
    ...
    min_student = get_min_student(students, n);
    max_student = get_max_student(students, n);
    average = get_average(students, n);

    printf("\n===== 결과 =====\n");
    printf("최저점 학생 정보\n");
    printf("이름: %s, 학번: %d, 성적: %d\n",
        min_student->name, min_student->id, min_student->score);

    printf("\n최고점 학생 정보\n");
    printf("이름: %s, 학번: %d, 성적: %d\n",
        max_student->name, max_student->id, max_student->score);
